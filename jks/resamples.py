...
class resamples:
    ...
    def add_gaussian(self, n, m, e):
        ...
        # TODO: add new tags
        self.set[n] = fnc( m, [ random.gauss(m,e / math.sqrt(self.N-1) ) for i in range(self.N) ] )
    def cut(self, i, keep_fixed = None):
        if i == -1:
            return dict( [ (t,self.set[t].orig) for t in self.set.keys() ] )
        else:
            if keep_fixed != None:
                return dict( [ (t,self.set[t].blocks[i] if t not in keep_fixed else self.set[t].orig) for t in self.set.keys() ] )
            else:
                return dict( [ (t,self.set[t].blocks[i]) for t in self.set.keys() ] )
    def _clone_type_fnc(self):
        return eval("lambda a,b,c,d: " + self._clone_type_str + "({ 'orig' : a, 'blocks' : b, 'tags' : c, 'info' : d })")
    def apply(self, phi, keep_fixed = None, scale=1.0 ):
        f=self._clone_type_fnc()
        if scale == 1.0:
            return f( phi(self.cut(-1)), [ phi(self.cut(i, keep_fixed)) for i in range(self.N) ], self.tags, self.info )
        else:
            x=self.cut(-1)
            y=numpy.array(phi(x))
            yi0=[ phi(dict([ (t,(numpy.array(xi)-numpy.array(x[t]))*scale + numpy.array(x[t])) for t,xi in self.cut(i, keep_fixed).items() ])) for i in range(self.N) ]
    ...
    def save(self, fn, compress=True):
        ...
        t0=time.time()
        s = { "N" : self.N, "set" : {}, "_clone_type_str" : self._clone_type_str, "tags" : self.tags, "info" : self.info,
              "origin" : { "argv" : sys.argv, "pwd" : os.getcwd(), "hostname" : socket.gethostname(), "user" : os.getlogin(),
                           "date" : str(datetime.datetime.now()), "argv[0].mtime" : str(datetime.datetime.fromtimestamp(os.path.getmtime(sys.argv[0]))) } }
        for f in self.set:
            s["set"][f] = { "orig" : self.set[f].orig, "blocks" : self.set[f].blocks, "N" : self.set[f].N }
    ...
    def load(self, fn, pre = ""):
        ...
        fnc=self._clone_type_fnc()
        for f in s["set"]:
            self.set[pre + f] = fnc( s["set"][f]["orig"], s["set"][f]["blocks"], self.tags, self.info )
    def used(self):
        used=set([])
        for f in self.set:
