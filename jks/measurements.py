...
def _read_files(fns,prepare):
    # read lines
    if prepare == None:
        data = [ [[float(v) for v in l.split(" ") if v != ""] for l in open(f).readlines()] for f in fns ]
    else:
        data = [ prepare([[float(v) for v in l.split(" ") if v != ""] for l in open(f).readlines()]) for f in fns ]
...
class measurements:
    ...
    def __init__(self,data,
        ...
        if type(data) == type({}):
            self.config_tags=sorted(list(data.keys())) # should be a list but make sure no future version gives a set
    ...
    def block(self,size):
        data=[]
        if self._NR % size != 0:
            raise Exception("Blocksize must divide number of measurements (%d / %d)" % (self._NR,size))
    ...
    def cov(self):
        return numpy.cov(m=self.ndata, rowvar=0, ddof=1).tolist() # unbiased estimate of covariance

    # correlation matrix Pij = Cij / sqrt(Cii * Cjj)
    def cor(self):
        return numpy.corrcoef(x=self.ndata, rowvar=0, ddof=1).tolist() # unbiased estimate of covariance
    ...
    def central_moment_biased(self,n):
        ...
        return numpy.mean(numpy.array([ numpy.power(d-m,n) for d in self.ndata ], self.np_datatype), axis=0)
    ...
    def cm4(self):
        ...
        d0=N**2 - 5*N + 6
        cm4=N*(9 - 6*N) / (N-1) / d0  * I22 + N*(3 - 2*N + N**2) / (N-1) / d0 * I4
    ...
    def ci(self):
        ...
        del3sqr=[ cm3[i]**2 for i in range(N) ] # TODO: should use unbiased estimator of cm3sqr
        ...
        L2_1sigma=[ 0.212683*del3sqr[i]/math.pow(sigma[i],5.0) - 0.08333333*del4[i]/math.pow(sigma[i],3.0) for i in range(N) ]
        L2_2sigma=[ 2.17095*del3sqr[i]/math.pow(sigma[i],5.0) + 0.08333333*del4[i]/math.pow(sigma[i],3.0) for i in range(N) ]
        ...
        return [
            [ (left_err1_0,right_err1_0), (left_err1_1,right_err1_1), (left_err1_2,right_err1_2) ],
            [ (left_err2_0,right_err2_0), (left_err2_1,right_err2_1), (left_err2_2,right_err2_2) ]
            ]
    ...
    class _jackknife:
        ...
        class _jk_resamples:
            def __init__(self, phi, jk = None):
                if jk == None:
                    ...
                    if jk.N > 1:
                        self.blocks = numpy.array([ phi(jk.master.eliminate(i*jk.blocksize,(i+1)*jk.blocksize)) for i in range(jk.N) ])
                    else:
            ...
            def measurements(self):
                ...
                return measurements([(numpy.array(self.blocks[i], measurements.np_datatype) - phiN).tolist() for i in range(self.N) if self.tags[i][0] != "!" ])
            def scaled_measurements(self):
                # this only does the statistical average, i.e., over tags that do not start with "!"
                blocks=[i for i in range(self.N) if self.tags[i][0] != "!" ]
                N=float(len(blocks))
                phiN=numpy.array(self.orig, measurements.np_datatype)
                r=measurements([(N*phiN - (N-1)*numpy.array(self.blocks[i], measurements.np_datatype)).tolist() for i in blocks ])
                r.config_tags = [ t for t in self.tags ]
                return r
            def bias(self):
                m=self.measurements()
                return (numpy.array(m.mean(), measurements.np_datatype)*(m._NR-1)).tolist()
            def cov(self, var=None):
                if var == None:
                    ...
                    if sm._NR==0:
                        return [ [ 0.0 for i in range(len(self.orig)) ] for j in range(len(self.orig)) ]
                    res=((sm._NR-1)*numpy.array(sm.cov(), measurements.np_datatype)).tolist()
                else:
                    evar="!" + var
                    assert(evar in self.tags)
                    res=(4.0*numpy.cov(m=numpy.array([ self.orig, self.blocks[self.tags.index(evar)] ], measurements.np_datatype), rowvar=0, ddof=0)).tolist()
                if type(res) != type([]):
                    res=[[res]]
                return res
            def tcov(self):
        ...
        def __init__(self,master,blocksize):
            ...
            if master._NR % blocksize != 0:
                raise Exception("Blocksize must divide number of measurements (%d / %d)" % (master._NR,blocksize))
        ...
        def bias(self,phi): # = phiBAR - phiBAR_biascorrected
            return ((self.N-1)*numpy.array(self.prepare(phi).measurements().mean(), measurements.np_datatype)).tolist()
        ...
        def pe(self,phi): # 1/(n-1)*\sum_i ( pe_i - <pe> )^2 = cov()
            ...
            return [phiN + (self.N-1)*(phiN - phi(self.master.eliminate(i*self.blocksize,(i+1)*self.blocksize)))/math.sqrt(self.N-1) for i in range(self.N)]
...
def jackknife(orig, blocks, tags = None, info = None):
    ...
    if info == None:
        info = tags
    return measurements._jackknife._jk_resamples({ "orig" : orig, "blocks" : blocks, "tags" : tags, "info": info })
